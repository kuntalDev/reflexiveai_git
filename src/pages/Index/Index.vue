<template>
    <div class="main_div ease-in-outs z-[1] relative md:bg-[#fff]">
        <div class="py-[30px] xxl:py-[60px] md:h-full">
            <div v-if="!messageSent" class="md:h-full md:flex md:flex-col md:justify-center">
                <div>
                    <div
                        class="w-[76px] h-[76px] flex justify-center items-center border border-solid border-[#D2DDEB] rounded-full m-auto">
                        <img src="/src/assets/images/black-logo.svg" alt="black-logo" />
                    </div>
                    <h1
                        class="Pattanakarn-Medium xl:text-[26px] xxl:text-[34px] font-medium text-[#070707] text-center pt-[10px] xxl:pt-[16px] pb-[40px] xxl:pb-[83px]">
            ...
            <div v-else class="py-[0px] md:py-[20px] xxl:py-[90px] px-[20px] sm:px-[40px] xl:px-[102px] xxl:px-[141px]">
                <div
                    class="chat_list_right text-right w-full max-w-[90%] md:max-w-[70%] ml-auto pb-[20px] md:pb-[30px] lg:pb-[40px] xl:pb-[60px]">
                    <p
                        class="Pattanakarn-Regular text-[14px] text-[#000] bg-[#EFF4FB] rounded-[6px] lg:rounded-[20px] px-[27px] py-[16px] inline-block text-left break-all">
                        {{ message }}</p>
                </div>
                <div
                    class="chat_list_left w-full max-w-[90%] md:max-w-[70%] flex pb-[20px] md:pb-[30px] lg:pb-[40px] xl:pb-[60px]" v-if="agentMessage">
                    <div
                        class="w-[58px] h-[58px] border border-solid border-[#D2DDEB] rounded-full flex justify-center items-center">
                        <img src="/src/assets/images/black-logo.svg" alt="logo">
                    </div>
                    <div class="pl-[10px] md:pl-[22px] text_width">
                        <p class="Pattanakarn-Regular text-[14px] text-[#000] leading-[25px] pb-[4px] break-all">
                            {{ agentMessage }}</p>
                        <button @click="copyMessage">
                            <img src="/src/assets/images/copy-icon.svg" alt="copy">
                        </button>
                        <p v-if="copyStatus" class="copy-status-message">{{ copyStatus }}</p>
                    </div>
                </div>
            </div>
        </div>
        <div
            class="input_width px-[20px] sm:px-[40px] xl:px-[110px] xxl:px-[153px] bg-[#fff] fixed pb-[20px] md:pb-[40px] bottom-[0px] z-[0]">
            <div class="relative input_custome rounded-[26px] flex justify-end border border-solid border-[#D2DDEB]">
                <!-- <div class="w-full max-w-[50px] flex justify-center items-center btn_position" ref="dropdown1"
                    :class="{ 'custom-class': isOverflowing }">
                    <button @click="toggleDropdown1">
                        <img src="/src/assets/images/links.svg" alt="links" />
                    </button>
                    <div v-if="isDropdownOpen1" @click.self="closeDropdown1"
                        class="absolute z-[9] left-0 bottom-[100%] border border-solid border-[#DADADA] w-full max-w-[250px] bg-[#fff] rounded-[6px]">
                        <div class="p-[6px]">
                            <button class="flex items-center p-[8px]">
                                ...
                                <label for="file-upload"
                                    class="custom-file-upload Pattanakarn-Regular text-[14px] text-[#000] cursor-pointer">
                                    Choose File
                                </label>
                                <input id="file-upload" type="file"
                                    class="hidden Pattanakarn-Regular text-[14px] text-[#000]" />
                            </button>
                        </div>
                    </div>
                </div> -->
                <textarea v-model="text_prompt" placeholder="Enter a prompt here"
                    class="textarea-custom placeholder:text-[#77818F] relative Pattanakarn-Medium w-full leading-normal text-[13px] font-medium text-[#77818F] pl-[30px] pt-[17px] pr-[10px] resize-none overflow-y-hidden border-none"
                    @input="resizeTextarea" @keydown.enter.prevent="sendMessage" @focus="handleFocus" @blur="handleBlur"
                    ref="textarea"></textarea>
                <button type="submit" class="w-[70px] md:w-[55px] flex justify-center items-center btn_position"
                    @click="sendMessage" :class="{ 'custom-class': isOverflowing }">
                    <img src="/src/assets/images/backgroundArrow.svg" alt="backgroundArrow" />
                </button>
                <div
                    class="send_btn absolute m-auto right-0 left-0 bottom-[3px] bg-[#fff] w-[99%] border-b-[1px] border-solid border-[#D2DDEB]">
...
export default {
    ...
    setup() {
        ...
        // Listen for the 'newChatClicked' event
    ...
    methods: {
        ...
        sendMessage() {
            if (this.text_prompt.trim()) {
                this.message = this.text_prompt;

                // call api for answer
                this.getPromptAnswer(this.message); 
                // call api for answer end
        ...
        async getPromptAnswer(msg){
            ...
            try {
                const response = await this.$axios({
                    method: 'post',
                    url: 'https://reflexive-backend.onrender.com/chat/conversation_dummy',
                    data: payload,
                    headers: {
                ...
                this.agentMessage = response.data.choices[0].message.content;
            } catch (error) {
